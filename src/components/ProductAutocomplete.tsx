...
export const ProductAutocomplete = ({
  ...
  useEffect(() => {
    ...
    // If term is empty, show top recent entries from history
    const filtered = term === "" 
      ? priceHistory 
    ...
    const getUnique = (list: PriceHistoryItem[]) => {
      return list.reduce((acc, current) => {
        ...
        if (!desc) return acc;
        const exists = acc.some(item => (item.description || "").trim().toLowerCase() === desc.toLowerCase());
        if (!exists) {
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      ...
      {isOpen && suggestions.length > 0 && (
        <div className="absolute z-50 w-full mt-2 bg-white border border-zinc-200 rounded-2xl shadow-2xl shadow-brand-500/10 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="px-3 py-2 bg-zinc-50 border-b border-zinc-100 flex items-center justify-between text-[10px] font-black uppercase text-zinc-400 tracking-wider">
            <span className="flex items-center gap-1">
              <History className="w-3 h-3 text-brand-500" /> Billing History Suggestions
            </span>
            <span>{suggestions.length} items</span>
          </div>
          <div className="p-1.5 max-h-64 overflow-y-auto" ref={listRef}>
            {suggestions.map((suggestion, index) => (
              <button
                key={index}
                type="button"
                className={`w-full flex items-center justify-between px-3.5 py-2.5 text-sm text-left rounded-xl transition-all group ${
                  index === activeIndex ? "bg-brand-50 text-brand-700 font-semibold" : "hover:bg-zinc-50"
                }`}
                onMouseDown={(e) => {
                  e.preventDefault();
                  onSelect(suggestion);
              ...
              >
                <div className="flex flex-col pr-2 min-w-0">
                  <span className="font-bold text-zinc-900 group-hover:text-brand-700 transition-colors truncate">
                ...
                <History className="h-3.5 w-3.5 text-zinc-300 group-hover:text-brand-500 transition-colors shrink-0" />
              </button>
            ))}
          </div>
        </div>
